...
export default function EventMappingTable() {
  ...
  useEffect(() => {
    base44.entities.EventMapping.list().then(r => { setRecords(r); setLoading(false); });
  }, []);
  ...
  const saveEdit = async () => {
    await base44.entities.EventMapping.update(editingId, editForm);
        ...
        {loading ? <div className="h-24 flex items-center justify-center text-muted-foreground text-sm">Loading…</div> : (
          <table className="w-full text-sm">
            <thead className="bg-muted text-muted-foreground text-xs">
              <tr>
                {["Event Product", "Event", "Cut", "kg/unit", "Cost/kg", "Status", ""].map(h => (
            ...
            <tbody>
              {records.map(r => editingId === r.id ? (
                <tr key={r.id} className="border-t bg-blue-50">
                  <td className="px-3 py-2 text-xs">{r.event_product}</td>
                  <td className="px-3 py-1.5"><Input className="h-7 text-xs w-32" value={editForm.event || ""} onChange={e => set("event", e.target.value)} /></td>
                ...
                <tr key={r.id} className="border-t hover:bg-muted/20 cursor-pointer" onClick={() => startEdit(r)}>
                  <td className="px-3 py-2 text-xs font-medium">{r.event_product}</td>
                  <td className="px-3 py-2 text-xs">{r.event}</td>
