...
export default function ImportHistoryTable({ refreshKey, importType }) {
  ...
  const load = async () => {
    ...
    setBatches(all.filter(b => b.status !== "cleared"));
    setLoading(false);
  };
  ...
  const handleClearFailed = async (batch) => {
    if (!confirm(`Hide failed import log for "${batch.source_file_name || batch.filename}"? Failed batches are already excluded from dashboard numbers.`)) return;
    setReverting(batch.id);
    await base44.entities.ImportBatch.update(batch.id, { status: "cleared" });
  ...
  const statusBadge = (status) => {
    switch (status) {
      case "imported": return <Badge className="bg-green-100 text-green-800 border-0 text-xs">Imported</Badge>;
      case "failed_validation": return <Badge className="bg-red-100 text-red-800 border-0 text-xs">Failed</Badge>;
      case "failed_save": return <Badge className="bg-red-100 text-red-800 border-0 text-xs">Save Failed</Badge>;
      case "reverted": return <Badge className="bg-gray-100 text-gray-600 border-0 text-xs">Reverted</Badge>;
  ...
  return (
    <Card className="shadow-none border">
      ...
      <CardContent className="p-0">
        {loading ? (
          <div className="h-20 flex items-center justify-center"><div className="w-5 h-5 border-2 border-muted border-t-primary rounded-full animate-spin" /></div>
        ) : !filtered.length ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-muted text-muted-foreground">
                <tr>{["File Name", "Months", "Rows", "Date Range", "Import Date", "Status", ""].map(h => <th key={h} className="px-3 py-2 text-left font-medium whitespace-nowrap">{h}</th>)}</tr>
              </thead>
              <tbody>
                {filtered.map(b => {
                  const failed = ["failed_validation", "failed_save"].includes(b.status);
                  return (
                    <tr key={b.id} className={`border-t hover:bg-muted/20 ${b.status === "reverted" ? "opacity-50" : ""}`}>
                      <td className="px-3 py-2 max-w-[180px] truncate font-medium" title={b.source_file_name || b.filename}>{b.source_file_name || b.filename || "—"}</td>
                      <td className="px-3 py-2 font-mono text-xs">{getMonths(b)}</td>
                      <td className="px-3 py-2 tabular-nums">{b.rows_saved ?? b.row_count ?? "—"}</td>
                      <td className="px-3 py-2 text-xs text-muted-foreground whitespace-nowrap max-w-[140px] truncate">{b.date_range_detected || "—"}</td>
                      ...
                      <td className="px-3 py-2">
                        {(b.status === "imported" || failed) && (
                          <Button size="icon" variant="ghost" className="h-6 w-6 text-muted-foreground hover:text-destructive" disabled={reverting === b.id} onClick={() => failed ? handleClearFailed(b) : handleRevert(b)} title={failed ? "Hide failed import log" : "Revert / delete this import"}>
                            {reverting === b.id ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />}
                          </Button>
                        )}
                      </td>
                    </tr>
