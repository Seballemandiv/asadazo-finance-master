...
export default function Dashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setDebugInfo({
        ...
        revertedRows: sales.filter(r => r.import_batch_id && !salesBatchIds.has(r.import_batch_id)).length,
  ...
  const debugComputed = useMemo(() => ({
    matchingCount: filteredSales.length,
    netSum: filteredSales.reduce((s, r) => s + (r.net_amount_ex_vat || r.net_ex_vat || 0), 0),
    grossSum: filteredSales.reduce((s, r) => s + (r.gross_amount_inc_vat || r.gross_inc_vat || 0), 0),
    vatSum: filteredSales.reduce((s, r) => s + (r.vat_amount || r.vat || 0), 0),
    feeSum: filteredTransactions.reduce((s, r) => s + Math.abs(Number(r.transaction_fee || 0)), 0),
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Select value={selectedMonth || "all"} onValueChange={setSelectedMonth}>
          ...
          <SelectContent>{availableMonths.map(m => <SelectItem key={m} value={m}>{formatMonth(m)}</SelectItem>)}<SelectItem value="all">All months</SelectItem></SelectContent>
        </Select>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><MetricCard title="Loans / Paybacks (cash)" value={`€${(metrics.loanInPayback || 0).toFixed(2)}`} icon={TrendingUp} color="slate" small /></div>
      ...
      <div className="border rounded-lg overflow-hidden">
        <button className="w-full flex items-center justify-between px-4 py-3 bg-slate-50 text-sm font-medium text-slate-700 hover:bg-slate-100" onClick={() => setShowDebug(v => !v)}><span>🔍 Data Source Debug</span>{showDebug ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</button>
        {showDebug && <div className="px-4 py-4 text-xs font-mono space-y-1 bg-white text-slate-700"><p className="font-semibold text-slate-900 mb-2">Entity counts</p><p>Active sales batches: <strong>{debugInfo.salesBatchCount}</strong></p><p>Active bank batches: <strong>{debugInfo.bankBatchCount}</strong></p><p>Active transaction batches: <strong>{debugInfo.transactionBatchCount}</strong></p><p>Active sales rows: <strong>{debugInfo.activeSalesRows}</strong></p><p>Active bank rows: <strong>{debugInfo.bankRows}</strong></p><p>Active transaction rows: <strong>{debugInfo.transactionRows}</strong></p><p>Rows excluded: <strong>{debugInfo.revertedRows}</strong></p><p>Selected month: <strong>{selectedMonth || "(none)"}</strong></p><p>Sales rows matching month: <strong>{debugComputed.matchingCount}</strong></p><p>Sum net sales: <strong>€{debugComputed.netSum.toFixed(2)}</strong></p><p>Sum gross sales: <strong>€{debugComputed.grossSum.toFixed(2)}</strong></p><p>Sum VAT: <strong>€{debugComputed.vatSum.toFixed(2)}</strong></p><p>Sum SumUp fees: <strong>€{debugComputed.feeSum.toFixed(2)}</strong></p></div>}
      </div>
    </div>
  );
}
