...
export default function Stock() {
  ...
  const saveSku = async (event) => {
    ...
    const payload = {
      ...
      kg_per_unit: n(skuForm.kg_per_unit),
      units_per_pack: 1,
      default_sale_price_inc_vat: n(skuForm.default_sale_price_inc_vat),
      current_fca_cost_per_kg: n(skuForm.current_fca_cost_per_kg),
      current_landed_cost_per_kg: n(skuForm.current_landed_cost_per_kg),
      status: "Active",
    };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div><h1 className="text-2xl font-bold text-foreground">Stock & Costing</h1><p className="text-muted-foreground text-sm mt-1">Product SKUs, stock batches, landed cost and vehicle trip allocation.</p></div>
      ...
      {tab === "skus" && <Panel title="Product SKU Master"><form onSubmit={saveSku} className="rounded-lg border bg-background p-4 mb-4 space-y-3"><div className="grid grid-cols-1 md:grid-cols-4 gap-3"><Field label="Internal SKU"><Input value={skuForm.sku} onChange={e => setSku("sku", e.target.value)} placeholder="A-VACIO-1KG" /></Field><Field label="Public name"><Input value={skuForm.public_name} onChange={e => setSku("public_name", e.target.value)} placeholder="Vacío parrillero - 1kg" /></Field><Field label="Supplier"><Select value={skuForm.supplier} onValueChange={v => setSku("supplier", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{SUPPLIERS.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></Field><Field label="Family / cut"><Input value={skuForm.product_family} onChange={e => setSku("product_family", e.target.value)} placeholder="Vacío" /></Field><Field label="Kg/unit"><Input value={skuForm.kg_per_unit} onChange={e => setSku("kg_per_unit", e.target.value)} /></Field><Field label="Sale price inc VAT"><Input value={skuForm.default_sale_price_inc_vat} onChange={e => setSku("default_sale_price_inc_vat", e.target.value)} /></Field><Field label="FCA €/kg"><Input value={skuForm.current_fca_cost_per_kg} onChange={e => setSku("current_fca_cost_per_kg", e.target.value)} /></Field><Field label="Landed €/kg"><Input value={skuForm.current_landed_cost_per_kg} onChange={e => setSku("current_landed_cost_per_kg", e.target.value)} /></Field></div><div className="flex justify-end"><Button size="sm" type="submit"><Plus className="w-4 h-4 mr-1" /> Save SKU</Button></div></form><Table headers={["SKU", "Public name", "Supplier", "Family", "Kg/unit", "Sale price", "FCA €/kg", "Landed €/kg", "Status"]} rows={skus.map(s => [s.sku, s.public_name, s.supplier, s.product_family || s.cut, n(s.kg_per_unit).toFixed(2), money(s.default_sale_price_inc_vat), money(s.current_fca_cost_per_kg), money(s.current_landed_cost_per_kg), s.status])} /></Panel>}
