...
export default function Stock() {
  ...
  const [lineBatchFilter, setLineBatchFilter] = useState("all");
  ...
  const load = async () => {
    ...
    setLines(lineRows.filter((r) => r.status !== "Inactive"));
  ...
  const filteredLines = useMemo(() => lines.filter((l) => {
    const text = `${l.batch_ref || ""} ${l.sku || ""} ${l.public_name || ""} ${l.supplier || ""} ${l.cut || ""}`.toLowerCase();
    return (!lineSearch || text.includes(lineSearch.toLowerCase())) && (lineBatchFilter === "all" || l.batch_ref === lineBatchFilter);
  }), [lines, lineSearch, lineBatchFilter]);
  ...
  const visibleLineIds = filteredLines.map((l) => l.id);
  const allVisibleLinesSelected = visibleLineIds.length > 0 && visibleLineIds.every((id) => selectedLineIds.includes(id));
  ...
  const resetLineForm = () => { setLineForm(emptyLine); setEditingLineId(null); };
  const editLine = (row) => {
    setEditingLineId(row.id);
    setLineForm({ batch_ref: row.batch_ref || "", sku: row.sku || "", public_name: row.public_name || "", kg_received: String(row.kg_received || 0), units_received: String(row.units_received || 0), fca_cost_per_kg: String(row.fca_cost_per_kg || 0) });
  };
  ...
  const assignedKg = lines.filter((l) => l.batch_ref === lineForm.batch_ref && l.id !== editingLineId).reduce((sum, l) => sum + num(l.kg_received), 0);
  const remainingKg = batchKg - assignedKg;
  const overAssignedAfterSave = selectedBatch && lineKg > remainingKg;
  ...
  const saveLine = async (event) => {
    ...
    if (editingLineId) {
      await base44.entities.StockBatchLine.update(editingLineId, payload);
      setLines((prev) => prev.map((l) => l.id === editingLineId ? { ...l, ...payload } : l));
      resetLineForm();
    } else {
  ...
  const toggleLine = (id) => setSelectedLineIds((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]);
  const toggleVisibleLines = () => setSelectedLineIds((prev) => allVisibleLinesSelected ? prev.filter((id) => !visibleLineIds.includes(id)) : Array.from(new Set([...prev, ...visibleLineIds])));
  const deleteSelectedLines = async () => {
    if (!selectedLineIds.length || !window.confirm(`Archive ${selectedLineIds.length} selected batch line(s)?`)) return;
    for (const id of selectedLineIds) await base44.entities.StockBatchLine.update(id, { status: "Inactive" });
    setLines((prev) => prev.filter((l) => !selectedLineIds.includes(l.id)));
    setSelectedLineIds([]);
  };
  ...
  return <div className="p-6 max-w-7xl mx-auto space-y-6">
    ...
    {tab === "lines" && <Panel title="Batch Lines"><form onSubmit={saveLine} className="rounded-lg border bg-background p-4 mb-4 space-y-3"><div className="flex items-center justify-between gap-3"><div className="text-sm font-medium">{editingLineId ? "Edit Batch Line" : "Add Batch Line"} · landed {money(lineLandedTotal)} · {money(lineLandedPerKg)}/kg</div>{editingLineId && <Button type="button" size="sm" variant="outline" onClick={resetLineForm}><X className="w-4 h-4 mr-1" /> Cancel edit</Button>}</div>{selectedBatch && <div className={`rounded-md border px-3 py-2 text-sm ${overAssignedAfterSave ? "border-amber-300 bg-amber-50 text-amber-800" : "bg-muted/30 text-muted-foreground"}`}>Batch total: {batchKg.toFixed(1)} kg · Assigned excluding this line: {assignedKg.toFixed(1)} kg · Remaining before this line: {remainingKg.toFixed(1)} kg{overAssignedAfterSave ? " · Warning: this will exceed the batch total. Update the Stock Batch if this is the real received quantity." : ""}</div>}<div className="grid grid-cols-1 md:grid-cols-4 gap-3"><Field label="Batch"><Select value={lineForm.batch_ref} onValueChange={(v) => setLineField("batch_ref", v)}><SelectTrigger><SelectValue placeholder="Choose batch" /></SelectTrigger><SelectContent>{batches.map((b) => <SelectItem key={b.id} value={b.batch_ref}>{b.batch_ref}</SelectItem>)}</SelectContent></Select></Field><Field label="SKU"><Select value={lineForm.sku} onValueChange={(v) => setLineField("sku", v)}><SelectTrigger><SelectValue placeholder="Choose SKU" /></SelectTrigger><SelectContent>{skus.map((s) => <SelectItem key={s.id} value={s.sku}>{s.sku}</SelectItem>)}</SelectContent></Select></Field><Field label="Product"><Input value={lineForm.public_name} onChange={(e) => setLineField("public_name", e.target.value)} /></Field><Field label="Kg received"><Input type="number" min="0" value={lineForm.kg_received} onChange={(e) => setLineField("kg_received", e.target.value)} /></Field><Field label="Packs / units"><Input type="number" min="0" value={lineForm.units_received} onChange={(e) => setLineField("units_received", e.target.value)} /></Field><Field label="FCA €/kg"><Input type="number" min="0" value={lineForm.fca_cost_per_kg} onChange={(e) => setLineField("fca_cost_per_kg", e.target.value)} /></Field><Field label="Allocated shared costs"><Input readOnly value={money(lineAllocatedTransport + lineAllocatedHandling + lineAllocatedVehicle + lineAllocatedOther)} /></Field><Field label="Landed €/kg"><Input readOnly value={money(lineLandedPerKg)} /></Field></div><div className="flex justify-end"><Button size="sm" type="submit"><Plus className="w-4 h-4 mr-1" /> {editingLineId ? "Update line" : "Save line"}</Button></div></form><div className="flex flex-wrap items-center gap-2 mb-3"><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input className="pl-9 w-72" placeholder="Filter batch line, SKU, product..." value={lineSearch} onChange={(e) => setLineSearch(e.target.value)} /></div><Select value={lineBatchFilter} onValueChange={setLineBatchFilter}><SelectTrigger className="w-48"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All batches</SelectItem>{batches.map((b) => <SelectItem key={b.id} value={b.batch_ref}>{b.batch_ref}</SelectItem>)}</SelectContent></Select><Button size="sm" variant="outline" onClick={toggleVisibleLines}>{allVisibleLinesSelected ? "Clear visible" : "Select visible"}</Button>{selectedLineIds.length > 0 && <Button size="sm" variant="destructive" onClick={deleteSelectedLines}><Trash2 className="w-4 h-4 mr-1" /> Delete selected ({selectedLineIds.length})</Button>}</div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted/50"><tr><th className="px-3 py-2 text-left"><input type="checkbox" checked={allVisibleLinesSelected} onChange={toggleVisibleLines} /></th>{["Batch", "SKU", "Product", "Supplier", "Kg", "FCA €/kg", "Allocated cost", "Landed total", "Landed €/kg", "Remaining", "Edit"].map((h) => <Th key={h}>{h}</Th>)}</tr></thead><tbody>{filteredLines.map((l) => <tr key={l.id} className="border-t"><td className="px-3 py-2"><input type="checkbox" checked={selectedLineIds.includes(l.id)} onChange={() => toggleLine(l.id)} /></td><Td>{l.batch_ref}</Td><Td strong>{l.sku}</Td><Td>{l.public_name}</Td><Td>{l.supplier}</Td><Td>{num(l.kg_received).toFixed(1)}</Td><Td>{money(l.fca_cost_per_kg)}</Td><Td>{money(num(l.allocated_transport) + num(l.allocated_handling) + num(l.allocated_vehicle) + num(l.allocated_other))}</Td><Td>{money(l.landed_total)}</Td><Td>{money(l.landed_cost_per_kg)}</Td><Td>{`${num(l.kg_remaining).toFixed(1)} kg`}</Td><td className="px-3 py-2"><Button size="sm" variant="outline" onClick={() => editLine(l)}><Edit2 className="w-4 h-4" /></Button></td></tr>)}</tbody></table>{filteredLines.length === 0 && <Empty />}</div></Panel>}
