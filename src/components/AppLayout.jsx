...
export default function AppLayout() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <main ref={mainRef} data-scroll-container="true" className="flex-1 overflow-auto pb-20 md:pb-0 native-scroll">
        <div className="md:hidden flex items-center justify-between gap-3 px-4 py-3 border-b bg-card sticky top-0 z-30 pt-[calc(env(safe-area-inset-top)+0.75rem)]">
          <div className="flex items-center gap-2 min-w-0">
            {showBack ? <button onClick={() => navigate(-1)} className="p-2 rounded-lg border hover:bg-muted" aria-label="Go back"><ChevronLeft className="w-5 h-5" /></button> : <div className="w-11 h-11" aria-hidden="true" />}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 md:hidden border-t bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
        <div className="grid grid-cols-5 gap-1 px-2 py-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)]">
          {MOBILE_PRIMARY.map(item => { const active = isActive(pathname, item.path); const Icon = item.icon; return <Link key={item.path} to={item.path} onClick={(e) => { if (active) { e.preventDefault(); scrollMainToTop(); } }} className={cn("flex flex-col items-center justify-center gap-1 rounded-lg py-2 text-[11px] font-medium", active ? "bg-primary text-primary-foreground" : "text-muted-foreground")}><Icon className="w-4 h-4" /><span>{item.short}</span></Link>; })}
