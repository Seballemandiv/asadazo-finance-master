...
export default function DashboardEvents() {
  ...
  const handleSeedEvents = async () => {
    const defaults = [
      ["Argentina vs Austria", "2026-06-22", "Atelier Code Noir, Amsterdam"],
      ["Latin Food Experience", "2026-06-26", "Amsterdam"],
      ["BounceSpace", "2026-07-03", "Amsterdam"],
      ["Festival Macumba", "2026-07-18", "Amsterdam"],
      ["El Asadazo - 2nd Edition", "2026-08-29", "Amsterdam"],
    ];
    const existing = new Set(events.map(e => e.name.toLowerCase()));
    const created = [];
    for (const [name, event_date, location] of defaults) {
      if (existing.has(name.toLowerCase())) continue;
      created.push(await base44.entities.AsadazoEvent.create({ name, event_date, location, status: "Planned", is_active: true }));
    }
    setEvents(prev => [...prev, ...created].sort(sortEvents));
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Asadazo · Event P&L and calendar</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Select value={selectedEventId} onValueChange={setSelectedEventId}>
            ...
            <SelectContent>
              ...
              <SelectItem value="unassigned">Unassigned event rows</SelectItem>
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
        Event rows are now linked to an event, not only to a month. Assign event costs in Review Bank and event revenue in Review Sales. Unassigned event rows: <strong>{unassignedCount}</strong>.
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="border rounded-lg bg-card p-4"><h2 className="font-semibold mb-3">Event Sales Lines</h2><div className="overflow-x-auto max-h-80"><table className="w-full text-xs"><thead className="bg-muted text-muted-foreground"><tr><th className="px-2 py-2 text-left">Date</th><th className="px-2 py-2 text-left">Event</th><th className="px-2 py-2 text-left">Product</th><th className="px-2 py-2 text-right">Net ex VAT</th><th className="px-2 py-2 text-right">COGS</th></tr></thead><tbody>{eventSales.map(r => <tr key={r.id} className="border-t"><td className="px-2 py-2">{(r.transaction_date || r.date || "").slice(0, 10)}</td><td className="px-2 py-2 max-w-[160px] truncate">{r.event_name || "Unassigned"}</td><td className="px-2 py-2 max-w-[220px] truncate">{r.product_name || r.product}</td><td className="px-2 py-2 text-right">€{rowNet(r).toFixed(2)}</td><td className="px-2 py-2 text-right">€{Number(r.meat_cogs || 0).toFixed(2)}</td></tr>)}</tbody></table>{!eventSales.length && <p className="text-sm text-muted-foreground text-center py-8">No event sales rows.</p>}</div></div>
        <div className="border rounded-lg bg-card p-4"><h2 className="font-semibold mb-3">Event Bank Costs</h2><div className="overflow-x-auto max-h-80"><table className="w-full text-xs"><thead className="bg-muted text-muted-foreground"><tr><th className="px-2 py-2 text-left">Date</th><th className="px-2 py-2 text-left">Event</th><th className="px-2 py-2 text-left">Reference</th><th className="px-2 py-2 text-right">Amount</th></tr></thead><tbody>{eventCosts.map(r => <tr key={r.id} className="border-t"><td className="px-2 py-2">{(r.date || "").slice(0, 10)}</td><td className="px-2 py-2 max-w-[160px] truncate">{r.event_name || "Unassigned"}</td><td className="px-2 py-2 max-w-[260px] truncate">{r.reference || r.payment_ref}</td><td className="px-2 py-2 text-right">€{Number(r.event_cost || r.amount_out || 0).toFixed(2)}</td></tr>)}</tbody></table>{!eventCosts.length && <p className="text-sm text-muted-foreground text-center py-8">No event bank costs.</p>}</div></div>
      </div>
    </div>
  );
}
