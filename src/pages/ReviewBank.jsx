...
function rowModule(r) {
  if (r.module) return r.module;
  if (r.channel === "Event" || r.cost_type === "Event Cost" || r.event_id) return "Event";
  if (r.channel === "Online Shop") return "Online Shop";
  if (r.channel === "Wholesale") return "Wholesale";
  return "Other";
}
function rowChannel(r) {
  if (r.channel && !["Online Shop", "Event", "Wholesale"].includes(r.channel)) return r.channel;
  if (r.cost_type === "Payment Employees") return "Payment Employees";
  if (r.cost_type === "Shipping Cost") return "Shipping";
  if (r.cost_type === "Meat Purchase") return "Stock / Supplier";
  if (r.cost_type === "Car rental NL") return "Car rental NL";
  if (r.cost_type === "Transport Spain to Amsterdam") return "Transport Spain to Amsterdam";
  return "Other";
}
...
export default function ReviewBank() {
  ...
  const handleAddManualRow = async () => {
    ...
    const event = module === "Event" ? events.find(e => e.name === window.prompt("Event name", events[0]?.name || "")) : null;
  ...
  const summary = useMemo(() => {
    ...
    return { rows: visible.length, ok: visible.filter(r => r.review_status === "OK").length, review: visible.filter(r => r.review_status === "To review").length, ignore: visible.filter(r => r.review_status === "Ignore").length, employee: visible.reduce((s, r) => s + Number(r.employee_payment || (r.cost_type === "Payment Employees" ? r.amount_out : 0) || 0), 0), operating: visible.reduce((s, r) => s + Number(r.operating_expenses || 0), 0), carRental: visible.reduce((s, r) => s + Number(r.car_rental_nl || 0), 0), spainTransport: visible.reduce((s, r) => s + Number(r.transport_spain_to_amsterdam || 0), 0), event: visible.reduce((s, r) => s + Number(r.event_cost || 0), 0), payouts: visible.filter(r => r.cost_type === "Payment Processor Payout").reduce((s, r) => s + Number(r.amount_in || 0), 0) };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <Card><CardContent className="p-0 overflow-hidden"><table className="w-full table-fixed text-sm"><thead className="bg-muted text-muted-foreground text-xs"><tr>{["Date", "Reference", "Amount", "Cost / Cash Type", "Module", "Channel", "Event", "Status", ""].map(h => <th key={h} className="px-2 py-2 text-left font-medium truncate">{h}</th>)}</tr></thead><tbody>{loading ? <tr><td colSpan={9} className="h-32 text-center text-sm text-muted-foreground">Loading...</td></tr> : filtered.map(r => editingId === r.id ? <BankRowEditor key={r.id} record={r} eventOptions={events} onSave={updates => handleUpdate(r.id, updates)} onRemove={handleRemoveFromFinance} onCancel={() => setEditingId(null)} /> : <tr key={r.id} className="border-t hover:bg-muted/20 cursor-pointer" onClick={() => setEditingId(r.id)}><td className="px-2 py-2 text-xs truncate">{r.date}</td><td className="px-2 py-2 text-xs truncate" title={r.reference || r.payment_ref}>{r.reference || r.payment_ref}</td><td className="px-2 py-2 text-xs truncate">{r.amount_out > 0 ? `-€${Number(r.amount_out || 0).toFixed(2)}` : r.amount_in > 0 ? `€${Number(r.amount_in || 0).toFixed(2)}` : ""}</td><td className="px-2 py-2 text-xs truncate">{r.cost_type}</td><td className="px-2 py-2 text-xs truncate">{rowModule(r)}</td><td className="px-2 py-2 text-xs truncate">{rowChannel(r)}</td><td className="px-2 py-2 text-xs truncate">{r.event_name || "—"}</td><td className="px-2 py-2"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusBadge[r.review_status] || ""}`}>{r.review_status}</span></td><td className="px-2 py-2 text-xs text-muted-foreground">Edit</td></tr>)}</tbody></table></CardContent></Card>
    </div>
  );
}

function Metric({ label, value, green, amber }) {
  return <div className="rounded-lg border p-3"><div className="text-muted-foreground text-xs">{label}</div><div className={`font-semibold ${green ? "text-green-700" : amber ? "text-yellow-700" : ""}`}>{value}</div></div>;
}
